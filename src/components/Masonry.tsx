      ...
      {grid.map((item, index) => (
        ...
        >
          <div className="relative w-full h-full rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="absolute inset-0 bg-linear-to-t from-black/95 via-black/50 to-black/10 opacity-0 transition-opacity duration-300 group-hover:opacity-100 pointer-events-none" />
            ...
            <div className="absolute inset-x-0 bottom-0 p-5 flex flex-col justify-end translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
              {(item.category || item.date) && (
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {item.date && (
                    <time className="text-white/80 text-xs font-semibold tracking-wide">
                      {item.date}
                    </time>
                  )}
                </div>
